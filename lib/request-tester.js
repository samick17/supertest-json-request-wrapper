(function() {
    ...
    function requestRetrieveTest(options) {
        ...
        let postAction = options.postAction || function(done) {done()};
        let testExecutor = this;
        it(testName, function(done) {
            testExecutor.requestAgent[method.toLowerCase()](routePath, status, data, () => {
                postAction(done);
            });
    ...
    function requestModifyTest(options) {
        ...
        let testExecutor = this;
        it(testName, function(done) {
            testExecutor.requestAgent[method.toLowerCase()](routePath, body, status, data, () => {
                postAction(done);
            });
        }).timeout(testExecutor.timeout);
    }
